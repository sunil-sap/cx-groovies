...
insertQuery = 'INSERT INTO %s (pk)\n' +
        'SELECT pk FROM users item_t0 WHERE ( item_t0.p_uid  like \'fi_%%\' OR item_t0.p_uid like \'mico-spa_%%\' OR item_t0.p_uid like \'stramit_%%\') AND (item_t0.TypePkString=8796096823378 ) ;';
    ...
    while (currentPage < pageCount) {
        ...
        if (toBeRemovedPKs) {
            ...
            if (!importResult.successful) {
                ...
                jdbcTemplate.execute("delete from userscleanup where pk in (${removeFromCleanupTable})")
...
createTable('users' ,'userscleanup' )
